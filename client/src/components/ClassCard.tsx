...
export default function ClassCard({ cls, progress }: Props) {
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-400 mb-3 line-clamp-2">{cls.description}</p>

      <div className="flex items-center justify-between text-xs text-slate-500">
        <span>{cls.lesson_count ?? 0} lessons</span>
        {progress && (
          <span className={`font-medium ${progress.status === 'completed' ? 'text-emerald-400' : 'text-indigo-400'}`}>
